    ...
    const char* expected) noexcept
{
    ...
    // BCryptHash needs a newer NTDDI floor than this plugin targets; the three-call form is
    // equivalent and available everywhere the game runs.
    BCRYPT_HASH_HANDLE hash = nullptr;
    if (status >= 0)
        status = BCryptCreateHash(algorithm, &hash, nullptr, 0, nullptr, 0, 0);
    if (status >= 0)
        status = BCryptHashData(hash, const_cast<PUCHAR>(bytes), static_cast<ULONG>(count), 0);
    if (status >= 0)
