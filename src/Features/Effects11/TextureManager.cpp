...
void TextureManager::CreateCommonTextures()
{
	...
	// Post pass ping-pong target, so TextureSDRTemp (its TextureOriginal) is never rendered into mid-chain
	commonTextureCache.insert({ "TextureSDRTemp3", CreateTexture(screenWidth, screenHeight, DXGI_FORMAT_R10G10B10A2_UNORM, "TextureManager::TextureSDRTemp3") });
