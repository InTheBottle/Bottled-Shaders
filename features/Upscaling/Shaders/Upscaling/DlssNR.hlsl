...
#endif
{
    ...
    uint  gReset;        // stabilise: last frame's edit does not belong to this scene
    float gStability;    // stabilise: share of the reprojected history kept, 0..0.95
    float gJitterDeltaX; // convert: change in sample-position jitter since last frame, guide pixels
    float gJitterDeltaY;
};
...
RWTexture2D<float2> gMotionOut : register(u2); // convert: motion for the model, pixels of the guide plus the jitter delta.
#ifdef VK_MODE
[[vk::binding(7, 0)]]
#endif
...
// A cubic B-spline read in four bilinear taps.
//
// The reduced model's edit has to be enlarged to the frame, and a bilinear enlargement of a 2x or 3x
// smaller picture is a field of little tents: every small pixel becomes a diamond whose peak sits on
// its centre, and as the scene moves the peaks slide under the frame's own detail. That is most of
// what reads as crawl at the lower resolutions. The B-spline is the smooth cubic, no ringing, and its
// sixteen-texel footprint collapses to four bilinear fetches when the offsets are chosen right.
float4 SampleBSpline(Texture2D<float4> tex, float2 uvq)
{
    uint w, h;
    tex.GetDimensions(w, h);
    const float2 size = float2(w, h);
    const float2 p = uvq * size - 0.5;
    const float2 f = frac(p);
    const float2 i = p - f;
    const float2 f2 = f * f;
    const float2 f3 = f2 * f;
    const float2 w0 = (1.0 / 6.0) * (-f3 + 3.0 * f2 - 3.0 * f + 1.0);
    const float2 w1 = (1.0 / 6.0) * (3.0 * f3 - 6.0 * f2 + 4.0);
    const float2 w2 = (1.0 / 6.0) * (-3.0 * f3 + 3.0 * f2 + 3.0 * f + 1.0);
    const float2 w3 = (1.0 / 6.0) * f3;
    const float2 g0 = w0 + w1;
    const float2 g1 = w2 + w3;
    const float2 t0 = (i - 1.0 + w1 / g0 + 0.5) / size;
    const float2 t1 = (i + 1.0 + w3 / g1 + 0.5) / size;
    return g0.x * g0.y * tex.SampleLevel(gLinear, float2(t0.x, t0.y), 0)
         + g1.x * g0.y * tex.SampleLevel(gLinear, float2(t1.x, t0.y), 0)
         + g0.x * g1.y * tex.SampleLevel(gLinear, float2(t0.x, t1.y), 0)
         + g1.x * g1.y * tex.SampleLevel(gLinear, float2(t1.x, t1.y), 0);
}
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    // Steadying the reduced model's edit before it is carried up. One thread per model pixel.
    //
    //   gSource    the frame shrunk to model size (what the model was shown)
    //   gModel     what the model returned
    //   gOriginal  last frame's steadied edit, at model size
    //   gMotion    the game's motion vectors, at guide size
    //   gTarget    this frame's model picture with the steadied edit, for the resolve
    //   gKeep      this frame's steadied edit, next frame's history
    //
    // The model re-decides its fine detail every frame, and at a third of the frame each decision is a
    // three-pixel blob, so the re-deciding is what flickers. The edit is blended with its own
    // reprojected history and the history is clamped to the neighbourhood of this frame's edit first,
    // so where the scene really changed, or the motion vector is wrong, it falls back to this frame
    // rather than dragging an old answer across. Upstream measured accumulation a dead end at full
    // resolution, where the model's answer is anchored to real detail; this runs only when it is not.
    if (gMode == 5)
    {
        // Motion for the model: the game's normalised vectors as pixels of the guide, plus the change
        // in sample-position jitter since last frame, so the model is not told the whole scene moved
        // by the jitter. This is the input the reference implementation hands its model.
        gMotionOut[id.xy] = gMotion.Load(int3(id.xy, 0)).xy * float2(gMvScaleX, gMvScaleY) + float2(gJitterDeltaX, gJitterDeltaY);
        return;
    }
    ...
    if (gMode == 4)
    {
        ...
        const int2 maxPx = int2(gWidth - 1u, gHeight - 1u);
        const float3 proxyHere = gSource.Load(int3(px, 0)).rgb;
        const float3 e = gModel.Load(int3(px, 0)).rgb - proxyHere;
        ...
        [unroll] for (int dy = -1; dy <= 1; ++dy)
        {
            [unroll] for (int dx = -1; dx <= 1; ++dx)
            {
                if (dx == 0 && dy == 0)
                    continue;
                const int2 q = clamp(px + int2(dx, dy), int2(0, 0), maxPx);
                const float3 n = gModel.Load(int3(q, 0)).rgb - gSource.Load(int3(q, 0)).rgb;
                mn = min(mn, n);
                mx = max(mx, n);
            }
        }
        ...
        float3 steadied = e;
        if (gReset == 0u && gStability > 0.0)
        {
            // The motion texture is guide-sized; read the texel under this model pixel. Vectors point
            // from where a pixel is to where it was, in the game's normalised units, and gMvScale turns
            // them into pixels of this dispatch.
            const int2 gp = clamp(int2(uv * float2(gGuideWidth, gGuideHeight)), int2(0, 0), int2(gGuideWidth - 1u, gGuideHeight - 1u));
            float2 mv = gMotion.Load(int3(gp, 0)).xy;
            if (!all(mv == mv))
                mv = float2(0.0, 0.0);
            const float2 prevUv = uv + mv * float2(gMvScaleX, gMvScaleY) / float2(gWidth, gHeight);
            if (all(prevUv >= 0.0) && all(prevUv <= 1.0))
            {
                float3 h = gOriginal.SampleLevel(gLinear, prevUv, 0).rgb;
                h = clamp(h, mn, mx);
                steadied = lerp(e, h, gStability);
            }
        }

        gKeep[id.xy] = float4(steadied, 1.0);
        gTarget[id.xy] = float4(proxyHere + steadied, 1.0);
        return;
    }
    ...
    // When the model ran below the frame, the edit is enlarged with the cubic rather than the bilinear
    // read above (the bilinear proxy and model are kept for the luminance comparisons, where a softer
    // read changes nothing that matters). At the frame's own size the two reads are identical.
    float3 edit = model - proxy;
    {
        uint editW, editH;
        gSource.GetDimensions(editW, editH);
        if (editW != gWidth || editH != gHeight)
        {
            float3 pC = SampleBSpline(gSource, cmpUv).rgb;
            float3 mC = SampleBSpline(gModel, cmpUv).rgb;
            if (gPassthrough == 0)
            {
                pC = SrgbToLinear(pC);
                mC = SrgbToLinear(mC);
    ...
    if (gTransfer == 1 && modelRanSmall)
    {
        ...
        // Passthrough hands the model the frame as it is, so the proxy it worked from was never
        // clamped and the rebuilt one must not be either: with a scene-linear frame the saturate
        // pins every bright sky pixel to 1.0, the luminances stop matching, and the composition
        // takes the headroom branch only below full resolution. Bottled Shaders change.
        float3 fullProxy = gPassthrough != 0 ? original : saturate(SoftKnee(original));
        proxy = fullProxy;
    ...
    // A model fed something it cannot read can return NaN or infinities; those must never reach
    // the ratio below or the frame. Fall back to the proxy, which makes the pass a no-op here.
    // Bottled Shaders change.
    if (any(isnan(model)) || any(isinf(model)))
        model = proxy;
